...
本项目按发布 tag 记录版本，完整提交见 git 历史。`alpha` 表示内部验证，`beta` 表示功能验证，`rc` 表示只接受阻断稳定发布的问题修复；只有不带预发布后缀的 SemVer tag 才是稳定版。

> 版本整理说明：2026-07-10 至 2026-07-11 的旧 `v2.0.0` 至 `v2.4.10` 均产生于 v2 尚未稳定的集中开发期，现按实际成熟度重编号为 `v2.0.0-alpha.1` 至 `v2.0.0-beta.12`。

## 2026-07-19 — v2.0.1-rc.3（管理端加载反馈与移动端排版收口）

- 概览首屏新增与真实布局对齐的响应式骨架屏，数据返回前展示状态条、主面板和指标占位，消除空白闪烁；加载状态包含屏幕阅读器提示，骨架动画继续遵循减弱动效偏好。
- 全局通知和活动连接列表接入 Vue `TransitionGroup`，补齐进入、离开与重排动画；通知移除和连接关闭后其余项目平滑归位，不再瞬间跳动。
- 系统性修复 320px、390px 与 768px 下的长文本布局：节点名、订阅与规则集标题、URL 以及资源列表标签均在容器内换行，网格文本列允许收缩，不再挤出按钮或产生横向溢出。
- 管理端构建产物同步更新，并通过移动端极端长文本压力截图、类型检查、前端构建与部署产物一致性回归。

## 2026-07-19 — v2.0.1-rc.2（国外服务规则集目录与管理端视觉体系升级）

- 管理端推荐规则集聚焦国外服务二次分流，补充 AI、Google、Telegram、Discord、流媒体、Emby、Steam、Apple/Microsoft 国际服务、下载/CDN、GitHub/GitLab、Reddit、Spotify、X/Twitter 和 TikTok 等分类；不改变国内/国际基础分流，也不安装即启用。
- 规则来源保留 RealSeek 和 blackmatrix7 的上游归属；新增 Clash classical `payload` 列表解析，只转换域名、关键词和 CIDR 条件，忽略进程名与 User-Agent 条件。
- 规则目录不推荐 Mihomo `.mrs`；二进制规则集需匹配项目固定的 sing-box 1.12.x 规则集版本，默认推荐文本源以避免版本不兼容。
- 管理端视觉体系升级：圆角与弹性缓动 token 化，徽章与协议标签扩展蓝/绿/橙/红/紫色调变体，连接页 TCP/UDP 按协议区分颜色；服务、资源与托管列表统一为 iOS 分组嵌入式卡片层级。
- 交互反馈打磨：按钮弹性按压回弹、卡片悬停描边、侧栏激活指示条、页面切换分节渐入；全部动效遵循减弱动效偏好。
- 空状态组件化：全部列表空状态改为图标加主副文案的引导块，错误信息改为醒目的红色色块；修复窄屏下分组列表残留分隔线与零内边距问题。

## 2026-07-16 — v2.0.1-rc.1（升级启动隔离与防火墙诊断）

- 修复旧防火墙未能迁移到动态 IP 白名单时，`pdg-admin` 的 `ExecStartPre` 因同步 unit 失败而阻断管理端启动、触发整次更新回滚的问题。
- IP 登记 socket 与过期文件清理继续正常启用；仅在动态 nft set 已存在时把白名单同步失败视为更新硬错误，旧防火墙则保留原来源限制并明确提示后续迁移。
- 防火墙模板校验和实际加载失败时直接输出原始 `nft` 错误，同时继续保留旧配置、旧表和原子回滚保证。
- 新增辅助迁移不阻断管理端、同步失败降级以及 `nft -c`/`nft -f` 错误透传回归。

## 2026-07-16 — v2.0.0（稳定版：动态 IP 白名单与访问收口）

- DNS `53/853` 与管理端 `9443` 改为仅允许动态 IP 白名单来源；`8111` 保留内网 CIDR 首次登记入口，其他数据端口继续限制在内网卡段。
- Telegram 客户端入口统一使用“IP 白名单”命名，支持 Android、iOS 和其他设备直接登记当前 IP；管理 API 新增规范的 `/api/v1/allowlist` 路径，并兼容原 iOS 路径与命令。
- iOS 描述文件链接默认 10 分钟失效，首次访问绑定客户端 IP；过期文件由定时器清理，默认描述文件也需首次登记，已登记 IP 可按设备或全部移除。
- nftables 重载、重启、更新和网段迁移后会从持久状态恢复白名单；同步失败会阻止产生不一致状态，并保留原配置或回滚。
- 旧防火墙迁移覆盖动态白名单上线前的 `inet pdg` 模板，继续跳过用户自定义规则；doctor、部署、API、Bot、下载服务和发布链路回归同步补齐。
- `v2.0.0-rc.1` 至 `rc.3` 的管理面板、订阅与策略组、分流、生命周期迁移及多协议兼容能力通过稳定性验收，正式发布 `v2.0.0`。

## 2026-07-16 — v2.0.0-rc.3(管理能力、生命周期与 iOS 下载安全)

- 管理面板补齐出口、订阅、策略组、域名/CIDR 分流、规则集、连接和资源操作，并统一 API 鉴权、安全响应头与全局错误反馈。
- Telegram Bot 与共享服务支持批量域名/CIDR 规则、规则顺序调整、订阅刷新、出口影响确认和异步长操作；Telegram 传输失败会重连，不再让轮询连接阻塞 API 请求。
- 扩展 Shadowsocks、VMess、VLESS、Hysteria/Hysteria2、TUIC、AnyTLS、ShadowTLS、SSH、SOCKS 和 HTTP 等链接及订阅格式兼容，并继续由 sing-box 1.12.x 做配置校验。
- 安装、更新、快照、回滚和卸载统一覆盖管理端、iOS 下载服务、Bot 凭据、TCP keepalive 与 journald 配置；老安装继续通过幂等迁移补齐服务和防火墙规则。
- iOS 描述文件改为 8111 socket 激活下载，网络层仅放行内网 CIDR，应用层再次校验来源 CIDR 和精确发布文件清单；服务使用动态用户和只读系统保护运行。
- 旧版本更新自举：`rc.2` 及更早更新器升级到本版本时，仅安装新 doctor 会导致 iOS 运行时新增文件未部署、校验失败并回滚；管理服务首次启动前会检查运行时，缺失时通过 `pdg migrate-ios-profile` 幂等补齐响应脚本、systemd unit、描述文件、精确文件清单、CIDR 白名单和防火墙端口，再继续 doctor 校验。
- 调整预发布版本策略：预发布 tag 在发现严重缺陷时可修复后重发同一个 tag，不重新开后缀号；稳定 `vMAJOR.MINOR.PATCH` 仍为不可变。
- 补齐管理 API、Bot 交互、链接解析、配置事务、迁移、iOS 下载、旧更新器自举、PWA 构建及部署产物一致性回归。

## 2026-07-11 — v2.0.0-rc.2(不可变发布标签与更新恢复)

- 发布标签改为不可变：已推送 tag 不再强制移动，后续修复通过递增 `rc.N` 发布，避免设备本地同名 tag 与远端对象冲突导致 `git fetch --tags` 中止。
- 标签同步在普通仓库和浅仓库展开阶段均使用明确的远端 refspec，并保留真实浅仓库回归。
- 管理端成功、错误、资源状态和运行态反馈统一为全局通知，固定显示在所有页面和弹出配置窗口之上；相同消息去重，错误支持手动关闭并自动消退。
- `v2.0.0-rc.1` 保留原提交引用，当前修复从 `v2.0.0-rc.2` 继续。

## 2026-07-11 — v2.0.0-rc.1(版本体系收口与规则优先级修复)

- 将 v2 集中开发期的版本统一整理为 `v2.0.0` 预发布序列，发布选择严格遵循 SemVer；标签同步会清理本地陈旧引用，并在浅仓库展开历史时同步远端现有标签。
- 手工域名、IP、端口、网络及协议规则统一优先于远程规则集，系统入站与动作规则仍保持最前，避免迁移改变 IP 分流结果。
- 规则集出口更新期间禁用重复操作；更新失败后恢复原选择，避免界面显示未实际生效的出口。
- 规则链接节点名兼容 `+` 空格编码，并同步规则资源管理界面、测试和 PWA 部署产物。

## 2026-07-11 — v2.0.0-beta.12(策略组术语与 Bot 出口概览)

- 将 PWA、Telegram Bot、共享服务错误及 README 中容易误解的“故障组”统一为“策略组”，创建和编辑场景统一使用“新建策略组”“保存策略组”。
- Telegram 首页将具体出口与策略组分区展示，具体出口逐行排列，不再将所有节点和策略组挤在同一行。
- 策略组明确显示“自动优选”或“固定：节点”模式，组内成员逐行展示，便于扫描成员较多的策略组。
- `/exits` 详情按具体出口和策略组分节；具体出口显示协议与地址，策略组显示模式、当前固定节点及成员。
- Bot 动态名称统一进行 HTML 转义；首页具体出口超过 40 个时折叠剩余数量，避免特殊字符破坏格式或触发 Telegram 消息长度限制。
- 更新 Bot 菜单示意和 CI 名称，新增状态摘要、出口详情、模式、换行与 HTML 转义回归，并重新同步 PWA 部署产物。

## 2026-07-11 — v2.0.0-beta.11(节点名称管理与操作反馈)

- 手动添加节点时可独立填写显示名称，无需修改链接中的名称片段；策略组和全部节点视图均提供统一改名入口及移动端底部编辑面板。
- 订阅节点支持持久化单节点别名，按协议、服务器、端口、TLS 服务名和传输定位信息绑定；订阅凭据变化或常规刷新后仍保留自定义名称。
- 节点改名通过原有配置事务同步迁移默认出口、策略组成员、Selector 固定选择、分流目标和 `detour` 引用，并同步订阅所有权元数据。
- 节点列表明确区分手动名称、订阅原名和订阅自定义名称；订阅来源卡片增加自定义名称数量及明确的更新时间。
- 单节点测速、单订阅/全部订阅刷新和规则目标更新增加独立进行中状态、按钮禁用和失败反馈，避免重复操作及整页误锁。
- 策略组、全部节点和分流规则筛选增加一键清除入口，移动端继续保持稳定尺寸、文本边界和视觉视口约束。
- 增加节点别名稳定性、改名引用迁移、管理 API、筛选清除和改名面板回归；完成 320/390/768/桌面 Playwright 验证并同步部署产物。

## 2026-07-11 — v2.0.0-beta.10(成熟面板资源与运行态工作台)

- 直接适配 MetaCubeXD 的策略组 master-detail 和节点行组件结构：桌面使用左侧组导航与右侧当前组工作区，移动端使用横向组导航，并提供组内搜索、地区/协议 facets、当前节点定位、测速和快捷切换。
- 直接适配 Zashboard 的 RuleCard 与可展开 ProxyGroup 交互：规则按顺序、类型、匹配内容和策略路径紧凑展示，目标为策略组时可原地展开成员、测速并切换当前节点。
- 概览页新增服务、资源异常、活动连接和更新时间状态条，默认策略与实时会话成为主面板，并提供节点、分流、连接、资源和运行管理快捷入口。
- 连接页增加主机/来源/规则/路径搜索、来源与网络筛选、时间/流量排序、暂停刷新和分组详情；连接只读 API 补充源/目标端口、嗅探主机、入站和匹配规则元数据。
- 资源页拆分为“状态、图库、覆写”三视图，系统日志增加搜索过滤；页面和工作区选择继续本地持久化。
- 随 PWA 分发 MetaCubeXD 与 Zashboard 的 MIT 许可证和改造说明，README 与仓库规范同步记录第三方资源归属。
- 增加策略组切换、规则内联展开、连接搜索/详情和全页面 320/390/768/桌面响应式回归；管理 API、配置事务和部署产物验证通过。

## 2026-07-11 — v2.0.0-beta.9(移动端工作台与交互优化)

- 针对 320px、390px 和 768px 重新约束节点、分流、资源、连接及系统页面，长订阅名、节点名、域名、主机名和日志会在容器内换行或截断，不再撑出页面。
- 移动端节点命令栏改为稳定的两行布局，策略组常用操作改用固定尺寸图标和 tooltip；视图标签、筛选控件、底部导航及规则目标选择适配极窄屏。
- 添加订阅、节点组和手工节点表单在移动端改为带遮罩的底部面板，限制在视觉视口内，支持背景滚动锁定、关闭按钮和 `Escape` 退出。
- 新增策略组一键展开/收起，并持久化当前一级页面、节点/规则工作区、排序方式、节点显示模式和组展开状态，刷新后恢复上次管理位置。
- 固化 PWA 的 320/390/768/桌面验证规范，部署回归增加移动端偏好、底部面板和极窄屏断点检查。
- 使用超长动态数据执行全页面 Playwright 截图及横向溢出检查，覆盖概览、节点、分流、资源、连接、系统和移动端底部面板。

## 2026-07-11 — v2.0.0-beta.8(策略组与分流工作台升级)

- 节点页新增“策略组、订阅来源、全部节点”三种工作视图；策略组总览可同时查看模式、成员健康、固定选择和引用，并直接执行组内测速、切换节点及默认出口。
- 节点只读 API 增加订阅归属和来源分类，管理端可按订阅、健康状态、名称及延迟筛选；订阅来源视图集中展示刷新状态、更新时间、策略组和节点预览。
- 分流页拆分为“规则、规则集”双工作区，提供规则类型与目标 facets、配置顺序/名称/目标排序，并将路由测试和新增表单改为按需展开。
- 规则集独立展示格式、可用性、更新时间、条目数量和目标策略，保留原有原子下载、校验、刷新及失败回滚流程。
- 内置 flags-only Twemoji Mozilla 字体，确保国旗标识不依赖终端系统字体；补充字体许可证与来源致谢。
- 增加来源归属、规则顺序和管理工作台部署回归，并通过桌面与 390px 移动端截图及横向溢出检查。

## 2026-07-11 — v2.0.0-beta.7(可读节点标签与管理工作台)

- 订阅节点直接保留原始名称，不再添加可见的 `sub_<hash>` 前缀；仅与手工节点、其他订阅或同订阅节点重名时追加短摘要。
- 附加分类组改用“主组名-分类名”的可读标签；旧前缀节点和分类组刷新时自动迁移，保留默认出口、分流引用、外部故障组成员及 selector 固定选择。
- 节点页重构为策略组导航和紧凑节点工作区，支持组范围浏览、搜索、名称/延迟排序、列表/卡片视图、批量测速及快捷固定节点。
- 分流页新增策略目标概览、类型与目标筛选、统一规则表格和内联出口调整；订阅、手工节点、故障组及规则集管理入口保持不变。
- 增加节点与分类组迁移、引用保持和工作台部署回归，管理 API、PWA 构建及发布链路验证通过。

## 2026-07-10 — v2.0.0-beta.6(订阅组名与 Reality 兼容修复)
...
## 2026-07-10 — v2.0.0-beta.5(订阅名称与添加流程优化)

- 节点和故障切换组名称支持国旗 emoji、中文及其他 Unicode 符号，订阅预览、配置标签和管理界面会保留原始地区标识。
- 添加订阅默认只显示完整 URL 和可选名称，过滤、分类、协议筛选、重命名及网络属性收进按需展开的高级设置。
- 完整保留订阅 URL 的查询参数，支持直接粘贴含多个参数的地址；管理 API 和界面继续只显示脱敏地址。
- 增加国旗名称、带参数 URL 和基础/高级表单分层回归，保持下载预览、确认应用、配置校验和失败回滚流程不变。

## 2026-07-10 — v2.0.0-beta.4(覆写预设与规则图库)

- 资源中心新增套餐信息清理、常用地区分组、名称整理和网络优化四套结构化覆写预设；套用后先展示订阅差异，确认后才写入配置。
- 新增常用规则集图库，提供 OpenAI、Telegram、Netflix、YouTube、GitHub、Apple、Microsoft 和游戏平台预设，兼容文本规则与 sing-box `.srs` 二进制规则。
- 订阅分类组和故障切换组支持中文及其他 Unicode 名称，统一执行 NFKC 规范化、UTF-8 长度限制和冲突校验；Bot 复用共享管理服务写入配置。
- README 明确项目用于专用移动网络的合法流量治理，并补充原项目、核心组件和规则数据来源致谢。

## 2026-07-10 — v2.0.0-beta.3(PWA 节点管控与资源中心)

- PWA 重组为概览、节点、分流、资源、连接和系统六个工作区，整合节点列表、三目标诊断测速、默认出口、组内自动优选/固定节点、连接终止和日志。
- 节点订阅新增结构化覆写：协议过滤、正则重命名、名称排序、TCP Fast Open 与 UDP 分片；刷新后覆写和组内固定选择继续生效，不执行远程脚本或任意 JSON Patch。
- 资源中心统一展示和刷新节点订阅、远程规则集、Geosite 与项目版本；项目升级复用 `pdg update` 的快照、校验和失败回滚。
- 移除 Zashboard 静态包、9443 Clash API 代理和所有入口；sing-box Clash API 继续只监听 `127.0.0.1:9090`，供管理服务读取运行数据和执行测速。

## 2026-07-10 — v2.0.0-beta.2(节点订阅、自动分类与安全刷新)

- PWA/API 新增节点订阅 CRUD，支持 Base64 URI 列表、纯文本 URI 列表和 SIP008 JSON；添加或修改前展示新增、更新、移除、跳过节点的脱敏差异预览。
- 每个订阅使用稳定标签前缀并自动生成独立 `urltest` 分类组；包含/排除正则可筛选地区或线路，管理 PWA 可显示、测速和切换生成的分类组。
- 刷新只更新订阅拥有的节点，手工节点不受影响；失效节点和订阅删除会迁移默认出口、域名/规则集路由、Telegram 路由及故障组引用。每日规则更新任务同时逐个刷新节点订阅，失败保留旧配置。
- 原始订阅令牌、节点链接和凭据仅保存在 600 权限元数据/配置文件中，API 只返回脱敏 URL 与节点摘要；单订阅限制 8MB/500 节点。

## 2026-07-10 — v2.0.0-beta.1(Zashboard 实时面板与快速分流管理)
...
## 2026-07-10 — v2.0.0-alpha.2(GMS route-options 兼容修复)
...
## 2026-07-10 — v2.0.0-alpha.1(PWA 管理端、统一控制层与 Bot 高频交互优化)
...
- 管理端复用 DoT 证书监听 9443,防火墙仅允许内网卡段;API 另用 600 权限随机令牌认证,Clash API 仍只监听 127.0.0.1。Bot 和 `sudo pdg admin` 可生成 fragment 带令牌入口,`sudo pdg admin --rotate` 可立即轮换泄露令牌。
- 新增 `deploy/bot/pdg_control.py`:sing-box 配置写入统一使用与 `pdg` CLI 相同的跨进程锁,先校验候选配置,再原子替换;服务启动不稳定时自动恢复上一版。
- Bot 添加出口改为「解析链接 → 脱敏预览 → 确认应用」,不再直接写配置或在消息里回显完整节点地址/凭据。
- 删除出口增加引用影响确认;默认出口、分流规则、故障组和 Telegram 专用出口的失效引用统一迁移到可用兜底出口。
- 输入格式或配置校验失败后保留当前操作,可直接修正重试;返回菜单、取消或切页仍会清理待确认数据。
- 一级菜单前置测域名、默认出口、出口和分流入口,更新操作收进运维菜单。
