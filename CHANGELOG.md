...
> 版本整理说明：2026-07-10 至 2026-07-11 的旧 `v2.0.0` 至 `v2.4.10` 均产生于 v2 尚未稳定的集中开发期，现按实际成熟度重编号为 `v2.0.0-alpha.1` 至 `v2.0.0-beta.12`。旧版本设备可通过一次性桥接 tag 无损升级到新的版本序列。

## 2026-07-11 — v2.0.0-rc.1(版本体系收口与规则优先级修复)

- 将 v2 集中开发期的版本统一整理为 `v2.0.0` 预发布序列，发布选择严格遵循 SemVer，并排除一次性迁移桥接 tag。
- 为已安装旧 `v2.4.10` 的设备提供兼容桥接发布；升级只替换项目代码和静态资源，保留现有配置、出口、订阅、令牌、证书和快照。第二代 `v2.4.12` 桥接会显式同步远端 tag 引用，清理设备残留的已删除旧 tag；第三代 `v2.4.13` 同时在浅仓库展开历史时强制同步已移动 tag，避免 Git 拒绝覆盖本地 RC tag 而中止更新。
